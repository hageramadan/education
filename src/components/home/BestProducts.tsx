...
import { fetchMostSellingProducts, calculateDiscountPercentage, getFullImageUrl, Product } from "@/services/api";
...
export function BestProducts() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const data = await fetchMostSellingProducts(1, 20);
      ...
      // ✅ إضافة type annotation للمعامل product
      const formattedProducts: FormattedProduct[] = firstEightProducts.map((product: Product) => {
      ...
      setPagination({
        ...
        total: Math.min(data?.pagination?.total || 0, 8)
      });
      setError(null);
    } catch (err) {
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const data = await fetchMostSellingProducts(nextPage, 20);
      ...
      // ✅ إضافة type annotation للمعامل product
      const newFormattedProducts: FormattedProduct[] = newProducts.map((product: Product) => {
        const originalPrice = product?.pricing?.price || 0;
        const finalPrice = product?.pricing?.final_price || 0;
        const discount = calculateDiscountPercentage(originalPrice, finalPrice);
